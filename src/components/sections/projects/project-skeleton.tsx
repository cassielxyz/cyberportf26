...
export const ProjectSkeleton = ({ className }: ProjectSkeletonProps) => {
  return (
    <Card
      className={cn(
        className,
        'relative border-cyan-300/50 bg-white/10 shadow-[0_0_15px_rgba(0,0,0,0.1)] backdrop-blur-xs dark:-teal-/50 dark:bg-gray-800/10 dark:shadow-[0_0_15px_rgba(0,0,0,0.3)]',
          ...
          <div className="space-y-2">
            ...
            <div className="grid w-full auto-cols-fr grid-flow-col grid-rows-5 gap-1">
              {Array.from({ length: 70 }).map((_, index) => (
                <Skeleton
                  key={index}
                  className="aspect-square rounded-sm border-cyan-300/50 dark:bg-gray-700/20"
                />
              ))}
            </div>
          </div>
